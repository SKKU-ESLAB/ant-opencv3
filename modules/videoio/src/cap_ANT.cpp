...
{
	DBusMessage* message;
	message = dbus_message_new_signal("/org/ant/camera/daemon", "org.ant.camera.daemon", msg);
	dbus_connection_send(conn, message, NULL);
	dbus_message_unref(message);
}
class CvCaptureCAM_ANT_CPP : CvCapture
{
				public:
					CvCaptureCAM_ANT_CPP();
					virtual ~CvCaptureCAM_ANT_CPP();
...
bool CvCaptureCAM_ANT_CPP::uinit_Semaphore(void)
{
	sem_close(this->sem);
//	sem_unlink(SEM_NAME);
	return true;
}
bool CvCaptureCAM_ANT_CPP::init_Semaphore(void)
{
	this->sem = sem_open(SEM_NAME, 0, 0666, 0);
	if(this->sem == SEM_FAILED)
	{
		fprintf(stderr, "[CvCaptureCAM_ANT_CPP::init_Semaphore] : Semaphore Initialization Error\n");
...
bool CvCaptureCAM_ANT_CPP::init_DBus(void)
{
	dbus_error_init(&error);
	connection = dbus_bus_get(DBUS_BUS_SYSTEM, &error);
	if(!connection)
	{
		fprintf(stderr, "[CvCaptureCAM_ANT_CPP::init_DBus] : dbus_bus_get Error\n"); 
...
bool CvCaptureCAM_ANT_CPP::readFrame(void)
{
...
void CvCaptureCAM_ANT_CPP::mainLoop(void)
{
				unsigned int count;
				count = 1;
				while(count-- > 0)
				{
								for(;;){
												...
												if(-1 == r)
												{
																if(EINTR == errno)
																		continue;
																fprintf(stderr, "[CvCaptureCAM_ANT_CPP::mainLoop] : Select Failed\n");
												}
												if(0 == r)
												{
																fprintf(stderr, "[CvCaptureCAM_ANT_CPP::mainLoop] : select timeout\n");
																break;
												}
...
CvCaptureCAM_ANT_CPP::CvCaptureCAM_ANT_CPP()
{
...
bool CvCaptureCAM_ANT_CPP::init_SharedMemorySpaceForProperty()
{
	shmid_for_status = shmget((key_t)SHM_KEY_FOR_STATUS, 0, 0);
	if(shmid_for_status == -1)
	{
		fprintf(stderr,"[CvCaptureCAM_ANT_CPP::init_SharedMemorySpaceForProperty] : Shmget Error\n");
		return false;
	}
	shmPtr_for_status = shmat(shmid_for_status, (void*)0, 0666|IPC_CREAT);
	if(shmPtr_for_status == (void*)-1)
	{
	  fprintf(stderr,"[CvCaptureCAM_ANT_CPP::init_SharedMemorySpaceForProperty] : Shmat Error\n");
		return false;
	}
...
bool CvCaptureCAM_ANT_CPP::init_SharedMemorySpace()
{
				shmid = shmget((key_t)SHM_KEY_FOR_BUFFER, 0, 0);
				if(shmid == -1)
				{
								fprintf(stderr, "[CvCaptureCAM_ANT_CPP::init_SharedMemorySpace] : Shmget Error\n");
								return false;
				}
				shmPtr = shmat(shmid, (void*)0, 0666|IPC_CREAT);
				if(shmPtr == (void*)-1)
				{
							fprintf(stderr, "[CvCaptureCAM_ANT_CPP::init_SharedMemorySpace] : Shmat Error\n");
							return false;
				}
...
bool CvCaptureCAM_ANT_CPP::uinit_SharedMemorySpace()
{
...
bool CvCaptureCAM_ANT_CPP::open(int index)
{	
				open_device(&fd);
			...
			//	frame = cvCreateImageHeader(cvSize(this->width, this->height), IPL_DEPTH_8U, 3);
				...
				if(!frame.imageData)
				{
					fprintf(stderr, "[CvCaptureCAM_ANT_CPP::open] : cvAlloc Error\n");
					return false;
				}
				for(;;){
				...
				if(!init_Semaphore())
				{
					fprintf(stderr, "[CvCaptureCAM_ANT_CPP::open] : Cannot Init Semaphore\n");
					return false;
				}
			  return true;
}
bool CvCaptureCAM_ANT_CPP::close()
{
// check if already running
	sendDbusMsg(this->connection, "OpenCVStop");			
	return true;			
}
/* get property Not Supported in ANT OpenCV lib */
double CvCaptureCAM_ANT_CPP::getProperty(int) const
{
				return false;
}
/* set Property Not Supported in ANT OpenCV lib */
bool CvCaptureCAM_ANT_CPP::setProperty(int, double)
{
				fprintf(stderr, "[CvCaptureCAM_ANT_CPP::setProperty] : Cannot configure the setProperty\n");
				return false;
}
bool CvCaptureCAM_ANT_CPP::grabFrame()
{
				mainLoop();				
				return true;
}
IplImage* CvCaptureCAM_ANT_CPP::retrieveFrame(int)
{
		  	unsigned offset;
				offset = buffer_size;
...
{
				CvCaptureCAM_ANT_CPP* capture = new CvCaptureCAM_ANT_CPP;
